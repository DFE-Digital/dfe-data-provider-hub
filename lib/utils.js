...
exports.getLatestRelease = function() {
	if (releaseUrl !== null) {
		...
		try {
			...
			releaseUrl = `https://github.com/alphagov/govuk-prototype-kit/archive/${
				data['name']
			}.zip`
...
exports.matchMdRoutes = function(req, res) {
	...
	if (
		fs.existsSync(path.join(__dirname, docsPath, req.params[0] + '.md'), 'utf8')
	) {
		var doc = fs.readFileSync(
			path.join(__dirname, docsPath, req.params[0] + '.md'),
			'utf8'
		)
		var html = marked(doc)
...
try {
	...
	// TODO: REMOVE
	console.warn(sessionDataDefaults)
} catch (e) {
	console.error(
